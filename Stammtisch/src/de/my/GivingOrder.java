...
public class GivingOrder {
    ...
    /**
     * Array to hold who is giving a present to whom. Iff receiver.get(from)=to, then from is giving a present to to.
     */
    private EnumMap<Person, Person> receiver = new EnumMap<Person, Person>(Person.class);

    /**
     * In this giving Order Person 'from' gives a present to 'to'. This will automatically switch who 'from' is giving a
     * present to, but it will not change other assignments.
     * 
     * @param from
     ...
     *             If from may not gift person to, e. g. to is the spouse of from.
    ...
    /**
     * This array stores who is allowed to give a present to whom. Iff ALLOWED[from][to] the 'from' may give a present
     * to 'to'. These are the overall rules and do not reflect the current state of the object.
     */
     ...
     * 
     * @return A string containing line breaks. You may use printLn to the print the String.
     */
    public String print() {
    ...
    /**
     * Output the name of the giver, a rotation number to use to decipher the receiver and the encrypted receiver name.
     * 
    ...
    /**
     * Returns a stream of Names and URLs, each URL should point to an image that encrypts the information who the guy
     * or gal with the given name has to gift.
     * 
     * @return Stream of Name, URL pairs. 
     */
    public Stream<AbstractMap.SimpleEntry<String, String>> getURLs() {
        Stream.Builder<AbstractMap.SimpleEntry<String, String>> builder;

        builder = Stream.builder();
        for (Person p : receiver.keySet()) {
            String giftReceiver = receiver.get(p).toString();
            
            final String urlPrefix="https://barcode.tec-it.com/barcode.ashx?data=";
            final String urlSuffix=".&code=MobileQRCode&multiplebarcodes=false&translate-esc=false&unit=Fit&dpi=96&imagetype=Gif&rotation=0&color=%23000000&bgcolor=%23fffffff&qunit=Mm&quiet=0&dmsize=Default&eclevel=L";
            
            final String receiverURL = urlPrefix+spaceToPercent20("Hallo "+p.toString()+". Du beschenkst in diesem Jahr "+giftReceiver)+urlSuffix;

            builder.add(new AbstractMap.SimpleEntry<String, String>(p.toString(), receiverURL));
        }
