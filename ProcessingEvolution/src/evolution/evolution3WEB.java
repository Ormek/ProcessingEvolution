...
final int SEED = 38;
final float WINDOW_SIZE = 1.0f; // window size multiplier.  If it's 1, the size is 1280x720.
...
final float SORT_ANIMATION_SPEED = 5.0f; // Determines speed of sorting animation.  Higher number is faster.
final float MINIMUM_NODE_SIZE = 0.4f; // Note: all units are 20 cm.  Meaning, a value of 1 equates to a 20 cm node.
...
final float AIR_FRICTION = 0.95f; // The lower the number, the more friction.  1 = no friction.  Above 1 = chaos.
final float MUTABILITY_FACTOR = 1.0f; // How fast the creatures mutate.  1 is normal.
...
public int rInt(){
  return PApplet.parseInt(random(-0.01f,1.01f));
}
class Rectangle{
  float x1, y1, x2, y2;
...
class Node{
  ...
  public void hitWalls(int index){
    Node ni = n.get(index);
    ...
    for(int i = 0; i < rects.size(); i++){
      ...
      if(abs(ni.x-(r.x1+r.x2)/2) <= (r.x2-r.x1+ni.m)/2 && abs(ni.y-(r.y1+r.y2)/2) <= (r.y2-r.y1+ni.m)/2){
        ...
        float rad = ni.m/2;
        float wallAngle = 0;
        if(distance <= 0.00000001f){ // distance is zero, can't use atan2
          if(section <= 2){
  ...
  public Node modifyNode(float mutability){
...
class Muscle{
  ...
  public void applyForce(int i, float target){
    Node ni1 = n.get(c1);
    Node ni2 = n.get(c2);
  ...
  public Muscle modifyMuscle(int nodeNum,float mutability){
    int newc1 = c1;
    int newc2 = c2;
    if(random(0,1)<0.02f*mutability*MUTABILITY_FACTOR){
      newc1 = PApplet.parseInt(random(0,nodeNum));
    }
    if(random(0,1)<0.02f*mutability*MUTABILITY_FACTOR){
      newc2 = PApplet.parseInt(random(0,nodeNum));
    }
    float newR = min(max(rigidity*(1+r()*0.9f*mutability*MUTABILITY_FACTOR),0.01f),0.08f);
    float maxMuscleChange = 1+0.025f/newR;
    float newCL = min(max(contractLength+r()*mutability*MUTABILITY_FACTOR,0.4f),2);
    float newEL = min(max(extendLength+r()*mutability*MUTABILITY_FACTOR,0.4f),2);
    ...
    if(random(0,1) < 0.5f){ //contractTime is changed
...
class Creature{
  ...
  public Creature modified(int id){
    ...
    creatureTimer+r()*16*mutability*MUTABILITY_FACTOR,min(mutability*MUTABILITY_FACTOR*random(0.8f,1.25f),2));
    ...
    if(random(0,1) < 0.04f*mutability*MUTABILITY_FACTOR || n.size() <= 2){ //Add a node
      modifiedCreature.addRandomNode();
    }
    if(random(0,1) < 0.04f*mutability*MUTABILITY_FACTOR){ //Add a muscle
      modifiedCreature.addRandomMuscle(-1,-1);
    }
    if(random(0,1) < 0.04f*mutability*MUTABILITY_FACTOR && modifiedCreature.n.size() >= 4){ //Remove a node
      modifiedCreature.removeRandomNode();
    }
    if(random(0,1) < 0.04f*mutability*MUTABILITY_FACTOR && modifiedCreature.m.size() >= 2){ //Remove a muscle
  ...
  public void addRandomMuscle(int tc1, int tc2){
    if(tc1 == -1){
      tc1 = PApplet.parseInt(random(0,n.size()));
      tc2 = tc1;
      while(tc2 == tc1 && n.size() >= 2){
        tc2 = PApplet.parseInt(random(0,n.size()));
      }
    }
    ...
    m.add(new Muscle(PApplet.parseInt(random(1,3)),tc1,tc2,rtime1,rtime2,
    min(rlength1,rlength2),max(rlength1,rlength2),isItContracted(rtime1,rtime2),random(0.02f,0.08f)));
  }
...
public void drawGround(int toImage){
  if(toImage == 0){
    noStroke();    
    fill(0,130,0);
    if(haveGround) rect(0,windowHeight*0.8f,windowWidth,windowHeight*0.2f);
    for(int i = 0; i < rects.size(); i++){
      Rectangle r = rects.get(i);
      rect(r.x1/camzoom-cam/camzoom+windowWidth/2,r.y1/camzoom+windowHeight*0.8f,(r.x2-r.x1)/camzoom,(r.y2-r.y1)/camzoom);
    }
  }else if(toImage == 2){
    popUpImage.noStroke();
    ...
    for(int i = 0; i < rects.size(); i++){
      Rectangle r = rects.get(i);
      popUpImage.rect(r.x1/camzoom-cam/camzoom+ww/2,r.y1/camzoom+wh*0.8f,(r.x2-r.x1)/camzoom,(r.y2-r.y1)/camzoom);
    }
  }
}
...
public void drawPosts(int toImage){
  if(toImage == 0){
    noStroke();
    for(int i = PApplet.parseInt((-cam*camzoom-windowWidth/2)/5)-1;
    i <= PApplet.parseInt((-cam*camzoom+windowWidth/2)/5)+1; i++){
      fill(255);
      rect(windowWidth/2+(i*5-cam-0.1f)/camzoom,windowHeight*0.8f-3/camzoom,0.2f/camzoom,3/camzoom);
      rect(windowWidth/2+(i*5-cam-1)/camzoom,windowHeight*0.8f-3/camzoom,2/camzoom,1/camzoom);
      fill(120);
    ...
    float w = 450;
    float h = 450;
    for(int i = PApplet.parseInt((-cam*camzoom-w/2)/5)-1;
    i <= PApplet.parseInt((-cam*camzoom+w/2)/5)+1; i++){
      popUpImage.fill(255);
      popUpImage.rect(w/2+(i*5-cam-0.1f)/camzoom,h*0.8f-3/camzoom,0.2f/camzoom,3/camzoom);
...
public void drawArrow(float x){
  noStroke();
  fill(120,0,255);
  rect(windowWidth/2+(x-cam-1.7f)/camzoom,windowHeight*0.8f-4.8f/camzoom,3.4f/camzoom,1.1f/camzoom);
  beginShape();
  ...
  endShape(CLOSE);
  fill(255);
  text((PApplet.parseFloat(round(x*2))/10)+" m",windowWidth/2+(x-cam)/camzoom,windowHeight*0.8f-3.91f/camzoom);
}
...
public boolean isItContracted(float rtime1, float rtime2){
  boolean contracted;
  if(rtime1 <= rtime2){
    return true;
  }else{
...
public void simulate(){
  for(int i = 0; i < m.size(); i++){
    Muscle mi = m.get(i);
    mi.thruPeriod = ((PApplet.parseFloat(timer)/cTimer)/PApplet.parseFloat(mi.period))%PApplet.parseFloat(1);
...
public void drawCreatureWhole(Creature cj, float x, float y, int toImage){
...
public void draw(){
  ...
  if(menu == 0){
    ...
    if(gen == -1){
      ...
      textAlign(CENTER);
      textAlign(RIGHT);
      text(PApplet.parseFloat(round(percentile.get(min(genSelected,percentile.size()-1))[14]*1000))/1000+" m",700,160);
