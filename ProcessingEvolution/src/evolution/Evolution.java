...
public class Evolution extends PApplet {
	...
	public static void main(String[] args) {
		PApplet.main("evolution.Evolution");
	}
	...
	final float SORT_ANIMATION_SPEED = 5.0f; // Determines speed of sorting
												...
												// faster.
	final float MINIMUM_NODE_SIZE = 0.4f; // Note: all units are 20 cm. Meaning,
											// a value of 1 equates to a 20 cm
											// node.
	final float MAXIMUM_NODE_SIZE = 0.4f;
	final float MINIMUM_NODE_FRICTION = 0.0f;
	final float MAXIMUM_NODE_FRICTION = 1.0f;
	final float GRAVITY = 0.005f; // higher = more friction.
	final float AIR_FRICTION = 0.95f; // The lower the number, the more
										// friction. 1 = no friction. Above 1 =
										// chaos.
	final float MUTABILITY_FACTOR = 1.0f; // How fast the creatures mutate. 1 is
											// normal.
	...
	final Rectangle[] RECTANGLES = { new Rectangle(2, -0.4f, 7, 1), // Example
																	// hurdles
			new Rectangle(4, -0.8f, 9, 1), new Rectangle(6, -1.2f, 11, 1), new Rectangle(8, -1.6f, 13, 1),
			new Rectangle(10, -2, 15, 1), new Rectangle(12, -2.4f, 17, 1), new Rectangle(14, -2.8f, 19, 1),
			new Rectangle(16, -3.2f, 21, 1), new Rectangle(18, -3.6f, 23, 1), new Rectangle(20, -4.0f, 25, 1) };

	float histMinValue = -1; // histogram information
	float histMaxValue = 20;
	...
	class Node {
		...
		void hitWalls(int index) {
			...
			for (int i = 0; i < rects.size(); i++) {
				...
				float px, py;
				int section = 0;
				if (abs(ni.x - (r.x1 + r.x2) / 2) <= (r.x2 - r.x1 + ni.m) / 2
						&& abs(ni.y - (r.y1 + r.y2) / 2) <= (r.y2 - r.y1 + ni.m) / 2) {
					...
					float rad = ni.m / 2;
					float wallAngle = 0;
					if (distance <= 0.00000001) { // distance is zero, can't use
													// atan2
						if (section <= 2) {
							wallAngle = PI / 4.0f + section * PI / 4.0f;
						} else if (section >= 6) {
							wallAngle = 5 * PI / 4.0f + (8 - section) * PI / 4.0f;
						} else if (section == 3) {
							wallAngle = PI;
		...
		Node modifyNode(float mutability) {
			float newX = x + r() * 0.5f * mutability * MUTABILITY_FACTOR;
			float newY = y + r() * 0.5f * mutability * MUTABILITY_FACTOR;
			float newM = m + r() * 0.1f * mutability * MUTABILITY_FACTOR;
			newM = min(max(newM, MINIMUM_NODE_SIZE), MAXIMUM_NODE_SIZE);
			float newF = f + r() * 0.1f * mutability * MUTABILITY_FACTOR;
	...
	class Muscle {
		...
		Muscle(int tperiod, int tc1, int tc2, double tcontractTime, float textendTime, float tcontractLength,
				float textendLength, boolean tcontracted, float trigidity) {
			period = tperiod;
			c1 = tc1;
			c2 = tc2;
			contractTime = (float) tcontractTime;
		...
		void applyForce(int i, float target) {
			...
			float force = (float) (min(max(1 - (distance / target), -0.4f), 0.4f));
		...
		Muscle modifyMuscle(int nodeNum, float mutability) {
			...
			float newR = (float) min(max(rigidity * (1 + r() * 0.9f * mutability * MUTABILITY_FACTOR), 0.01f), 0.08f);
			float maxMuscleChange = 1f + 0.025f / newR;
			float newCL = (float) min(max(contractLength + r() * mutability * MUTABILITY_FACTOR, 0.4f), 2);
			float newEL = (float) min(max(extendLength + r() * mutability * MUTABILITY_FACTOR, 0.4f), 2);
			float newCL2 = (float) min(newCL, newEL);
			float newEL2 = (float) min(max(newCL, newEL), newCL2 * maxMuscleChange);
			...
			return new Muscle((int) max(period + rInt(), 0), newc1, newc2, newCT, newET, newCL2, newEL2,
					isItContracted(newCT, newET), newR);
		}
	}
	...
	class Creature {
		...
		Creature modified(int id) {
			Creature modifiedCreature = new Creature(id, new ArrayList<Node>(n.size()), new ArrayList<Muscle>(m.size()),
					0, true, creatureTimer + r() * 16 * mutability * MUTABILITY_FACTOR,
					min(mutability * MUTABILITY_FACTOR * random(0.8f, 1.25f), 2));
			...
			if (random(0, 1) < 0.04 * mutability * MUTABILITY_FACTOR || n.size() <= 2) { // Add
																							// a
																							// node
				modifiedCreature.addRandomNode();
			}
			if (random(0, 1) < 0.04 * mutability * MUTABILITY_FACTOR) { // Add a
																		// muscle
				modifiedCreature.addRandomMuscle(-1, -1);
			}
			if (random(0, 1) < 0.04 * mutability * MUTABILITY_FACTOR && modifiedCreature.n.size() >= 4) { // Remove
																											// a
																											// node
				modifiedCreature.removeRandomNode();
			}
			if (random(0, 1) < 0.04 * mutability * MUTABILITY_FACTOR && modifiedCreature.m.size() >= 2) { // Remove
																											// a
																											// muscle
		...
		void addRandomNode() {
			...
			float y = n.get(parentNode).y + sin(ang1) * 0.5f * distance;
		...
		void addRandomMuscle(int tc1, int tc2) {
			...
			if (tc1 != -1) {
				...
				float ratio = random(0.01f, 0.2f);
				rlength1 = distance * (1 - ratio);
				rlength2 = distance * (1 + ratio);
			}
			m.add(new Muscle((int) random(1, 3), tc1, tc2, rtime1, rtime2, min(rlength1, rlength2),
	...
	void drawGround(int toImage) {
		if (toImage == 0) {
			...
			for (int i = 0; i < rects.size(); i++) {
				Rectangle r = rects.get(i);
				rect(r.x1 / camzoom - cam / camzoom + windowWidth / 2, r.y1 / camzoom + windowHeight * 0.8,
			...
			for (int i = 0; i < rects.size(); i++) {
				Rectangle r = rects.get(i);
				popUpImage.rect(r.x1 / camzoom - cam / camzoom + ww / 2, (float) (r.y1 / camzoom + wh * 0.8),
						(r.x2 - r.x1) / camzoom, (r.y2 - r.y1) / camzoom);
			}
		}
	}
	...
	void drawPosts(int toImage) {
		if (toImage == 0) {
			noStroke();
			for (int i = (int) ((-cam * camzoom - windowWidth / 2) / 5)
					- 1; i <= (int) ((-cam * camzoom + windowWidth / 2) / 5) + 1; i++) {
				fill(255);
				rect(windowWidth / 2 + (i * 5 - cam - 0.1) / camzoom, windowHeight * 0.8 - 3 / camzoom, 0.2 / camzoom,
						3 / camzoom);
				rect(windowWidth / 2 + (i * 5 - cam - 1) / camzoom, windowHeight * 0.8 - 3 / camzoom, 2 / camzoom,
						1 / camzoom);
				fill(120);
			...
			float w = 450;
			float h = 450;
			for (int i = (int) ((-cam * camzoom - w / 2) / 5) - 1; i <= (int) ((-cam * camzoom + w / 2) / 5) + 1; i++) {
				popUpImage.fill(255);
	...
	void drawArrow(float x) {
		noStroke();
		fill(120, 0, 255);
		rect(windowWidth / 2 + (x - cam - 1.7) / camzoom, windowHeight * 0.8 - 4.8 / camzoom, 3.4 / camzoom,
				1.1 / camzoom);
		beginShape();
		...
		endShape(CLOSE);
		fill(255);
		text((round(x * 2)) / 10 + " m", windowWidth / 2 + (x - cam) / camzoom, windowHeight * 0.8 - 3.91 / camzoom);
	}

	private void text(String string, double f, double d) {
		text(string, (float) f, (float) d);
	}

	private void vertex(double d, double e) {
		vertex((float) d, (float) e);

	}

	private void rect(double d, double e, double f, double g) {
		rect((float) d, (float) e, (float) f, (float) g);
	...
	public void draw() {
		...
		if (menu == 0) {
			...
			if (gen == -1) {
				...
				textAlign(CENTER);
				textAlign(RIGHT);
				text(round(percentile.get(min(genSelected, percentile.size() - 1))[14] * 1000) / 1000 + " m", 700, 160);
				...
				for (int x = 0; x < 40; x++) {
					...
					for (int i = 0; i < nodeNum; i++) {
						n.add(new Node(random(-1, 1), random(-1, 1), 0, 0, random(MINIMUM_NODE_SIZE, MAXIMUM_NODE_SIZE),
								random(MINIMUM_NODE_FRICTION, MAXIMUM_NODE_FRICTION))); // replaced
																						// all
																						// nodes'
																						// sizes
																						// with
					...
					c[y * 40 + x] = new Creature(y * 40 + x + 1, new ArrayList<Node>(n), new ArrayList<Muscle>(m), 0,
							true, heartbeat, 1.0f);
		...
		if (menu == 5) { // simulate running
			...
			if (timer == 900) {
				...
				c[creaturesTested].d = average * 0.2f;
			}
			if (timer >= 1020) {
				setMenu(4);
				creaturesTested++;
		...
		if (menu == 6) {
			// sort
			c2 = new ArrayList<Creature>(1000);
		...
		if (menu == 8) {
			...
			camzoom = 0.12f;
			background(220, 253, 102);
			float transition = 0.5f - 0.5f * cos(min(timer / 60, PI));
			for (int j = 0; j < 1000; j++) {
				Creature cj = c2.get(j);
		...
		if (menu == 12) { // Reproduce and mutate
			...
			for (int j = 0; j < 500; j++) {
				...
				c2.set(999 - j2, cj.modified(cj2.id + 1000)); // mutated
																// offspring 1
				n = c2.get(999 - j2).n;
				m = c2.get(999 - j2).m;
	...
	private void simulateAllCreatures() {
		Simulator simulator = new Simulator(c[0]);
		for (int i = 0; i < 1000; i++) {
			simulator.setTo(c[i]);
			simulator.simulate(900);
			average = simulator.getAverage();
			c[i].d = average * 0.2f;
		}
	}

	private void simulateAllCreaturesGlobal() {
		for (int i = 0; i < 1000; i++) {
