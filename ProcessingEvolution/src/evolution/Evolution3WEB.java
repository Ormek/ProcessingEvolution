...
public class Evolution3WEB extends PApplet {
	...
	final float SORT_ANIMATION_SPEED = 5.0f; // Determines speed of sorting
												...
												// faster.
	final static float MINIMUM_NODE_SIZE = 0.4f; // Note: all units are 20 cm.
													// Meaning, a value of 1
													// equates to a 20 cm node.
	final static float MAXIMUM_NODE_SIZE = 0.4f;
	final static float MINIMUM_NODE_FRICTION = 0.0f;
	final static float MAXIMUM_NODE_FRICTION = 1.0f;
	final static float GRAVITY = 0.005f; // higher = more friction.
	final static float AIR_FRICTION = 0.95f; // The lower the number, the more
												// friction. 1 = no friction.
												// Above 1 = chaos.
	final static float MUTABILITY_FACTOR = 1.0f; // How fast the creatures
													// mutate. 1 is normal.

	final static boolean haveGround = true; // true if the ground exists, false
											// if no ground.
	...
	final Rectangle[] RECTANGLES = {
			/*
			 * new Rectangle(2,-0.4,7,1), //Example hurdles new
			 * Rectangle(4,-0.8,9,1), new Rectangle(6,-1.2,11,1), new
			 * Rectangle(8,-1.6,13,1), new Rectangle(10,-2,15,1), new
			 * Rectangle(12,-2.4,17,1), new Rectangle(14,-2.8,19,1), new
			 * Rectangle(16,-3.2,21,1), new Rectangle(18,-3.6,23,1), new
	...
	static class Node {
		...
		public void hitWalls(Iterable<Rectangle> rects) {
			...
			for (Rectangle r : rects) {
				...
				float px, py;
				int section = 0;
				if (abs(x - (r.x1 + r.x2) / 2) <= (r.x2 - r.x1 + m) / 2
						&& abs(y - (r.y1 + r.y2) / 2) <= (r.y2 - r.y1 + m) / 2) {
	...
	class Muscle {
		...
		public Muscle modifyMuscle(int nodeNum, float mutability) {
			...
			return new Muscle(max(period + rInt(), 0), newc1, newc2, newCT, newET, newCL2, newEL2,
	...
	class Creature {
		...
		public Creature modified(int id) {
			Creature modifiedCreature = new Creature(id, new ArrayList<Node>(0), new ArrayList<Muscle>(0), 0, true,
					creatureTimer + r() * 16 * mutability * MUTABILITY_FACTOR,
			...
			if (random(0, 1) < 0.04f * mutability * MUTABILITY_FACTOR || n.size() <= 2) { // Add
																							// a
																							// node
				modifiedCreature.addRandomNode();
			}
			if (random(0, 1) < 0.04f * mutability * MUTABILITY_FACTOR) { // Add
																			// a
																			// muscle
				modifiedCreature.addRandomMuscle(-1, -1);
			}
			if (random(0, 1) < 0.04f * mutability * MUTABILITY_FACTOR && modifiedCreature.n.size() >= 4) { // Remove
																											// a
																											// node
				modifiedCreature.removeRandomNode();
			}
			if (random(0, 1) < 0.04f * mutability * MUTABILITY_FACTOR && modifiedCreature.m.size() >= 2) { // Remove
																											// a
																											// muscle
	...
	public void simulate() {
		for (int i = 0; i < m.size(); i++) {
			Muscle mi = m.get(i);
			mi.thruPeriod = ((PApplet.parseFloat(timer) / cTimer) / PApplet.parseFloat(mi.period))
